...
$sql="SELECT MAX(id_factu) FROM detalle_fac";
		$resultado1 = $conexion->query($sql) or die($conexion->error);
		$rows=$resultado1->num_rows;
		
		if ($rows>0) {	
			$row=$resultado1->fetch_assoc();
			$idFactura=$row['MAX(id_factu)'];
		

$consulta2="SELECT * FROM detalle_fac WHERE id_factu=$idFactura";
		$resultado2 = $conexion->query($consulta2) or die($conexion->error);
		$rows=$resultado2->num_rows;
		if ($rows>0) {	
$consulta3="SELECT sum(subtotal) as total from detalle_fac where id_factu=$idFactura";
